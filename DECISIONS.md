...
## ADR-011：內部 namespace `BittleApp` → `DogLabApp`

**Status**: Accepted（2026-04-28）
**Context**：ADR-010 把產品名改為 DogLab Coding，但內部 JS namespace 仍是 `BittleApp`，技術債。

### Options

| 選項 | 優點 | 缺點 |
|---|---|---|
| `App` | 最短 | 易與 React/Vue 等框架的 `App` 衝突 |
| `Lab` | 短而中性 | 不夠具體 |
| `DogLab` | 與品牌一致 | 略長 |
| **`DogLabApp`** | 與品牌一致、明確是 app 物件 | 多 3 字元 |

### Decision

選 **`DogLabApp`**。107 處 `BittleApp` 一次 replace_all 完成。

### Rationale

1. 與 ADR-010 品牌名稱一致
2. 明確語意（`xxxApp` 是常見的 vanilla JS 全域物件命名）
3. 不易與第三方框架衝突
4. 即使將來開源成 npm package，namespace 也清楚

### Consequences

- ✅ 9 個 .js 檔全部更新
- ✅ 內外名稱一致
- ❌ JS 檔名 `bittle-blocks.js` 等仍保留（這些是「Bittle 機器人」的積木，名稱合理）

---

## ADR-012：事件積木採「手動觸發」模式（vs polling vs 實機監聽）

**Status**: Accepted（2026-04-28）
**Context**：v0.5.0 加事件積木（lifted/dropped/touch/distance/voltage hat blocks）。事件如何被觸發？

### Options

| 方案 | 模擬模式 | 實機模式 | 開發複雜度 |
|---|---|---|---|
| **A. 手動觸發**（UI 按鈕 + console API）| 👆 點按鈕觸發 | 同 | 低 |
| B. Polling（每 200ms 詢問機器人狀態）| 模擬難實作 | setInterval 詢問 IMU/超音波 | 中 |
| C. 實機監聽（serial 收到回報）| 模擬無法測 | Serial 收到 `lifted` → 觸發 | 高 |

### Decision

v0.5.0 採 **方案 A（手動觸發）**。模擬模式 + 實機模式都用 5 個浮動按鈕觸發。

### Rationale

1. **教學優先**：學生能立刻理解「點按鈕 = 事件發生」
2. **離線可測**：不需實機就能驗證事件邏輯
3. **後續可加 polling**（v0.7+）— 不必現在做
4. **API 一致**：`DogLabApp.events.trigger('lifted')` 在 console 也能用

### Implementation

- `DogLabApp.events.{on, trigger, reset}` 三方法
- Hat block 用 `appendStatementInput('DO')` 接收動作積木
- Generator 包成 `DogLabApp.events.on('lifted', async () => { ... })`
- Run 按鈕開始時 `events.reset()` 避免殘留
- index.html 加 5 個 `.btn-event` 觸發按鈕
